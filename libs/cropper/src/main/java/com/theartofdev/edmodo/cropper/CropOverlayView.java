...
public class CropOverlayView extends View {
  ...
  private void drawBackground(Canvas canvas) {
    ...
    if (mCropShape == CropImageView.CropShape.ROUNDED_RECTANGLE) {
      float rx = rect.width();
      ...
      mPath.addRoundRect(rect, rx/5, ry/5, Path.Direction.CW);
//      mPath.addOval(mDrawRect, Path.Direction.CW);
      canvas.save();
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
        canvas.clipOutPath(mPath);
  ...
  private void drawBorders(Canvas canvas) {
    if (mBorderPaint != null) {
      ...
      if (mCropShape == CropImageView.CropShape.ROUNDED_RECTANGLE) {
        // Draw rounded rectangle crop window border
        float rx = rect.width();
        float ry = rect.height();
        canvas.drawRoundRect(rect, rx/5, ry/5, mBorderPaint);
      }
      else {
